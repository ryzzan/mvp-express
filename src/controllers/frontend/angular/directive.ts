...
export class Directive {
    ...
    for (const key in object) {
      if (Object.prototype.hasOwnProperty.call(object, key)) {
        if (key === 'form') {
          ...
          if (form) {
            const formBuilderName = this.sharedFunction.setIdToPropertyName(
              form?.id,
            );
            const formClassName = this.sharedFunction.setIdToClassName(
              form?.id,
            );
            const formPropertyName = this.sharedFunction.setIdToPropertyName(
              form?.id,
            );
            const formMetaObject = {
              builderName: formBuilderName,
        ...
        if (key === 'table') {
          ...
          if (table) {
            ...
            const tableClassName = this.sharedFunction.setIdToClassName(
              table.id,
            );
            const tablePropertyName = this.sharedFunction.setIdToPropertyName(
              table.id,
            );
                ...
                elements.forEach(tag => {
                  if (tag.row) {
                    if (tag.row.menu) {
                      ...
                      menus.forEach(menu => {
                        if (menu.dialog) {
                          const dialogTemplateAsClassName =
                            this.sharedFunction.setIdToClassName(
                              menu.dialog.templateFolder,
                            );
    ...
    for (const property in object) {
      if (Object.prototype.hasOwnProperty.call(object, property)) {
        if (property === 'form') {
          ...
          const formIdAsPropertyName = formObject
            ? this.sharedFunction.setIdToPropertyName(formObject.id)
            : '';
        ...
        if (property === 'table') {
                ...
                if (tag.row.menu) {
                  ...
                  menus.forEach(menu => {
                    if (menu.dialog) {
                      ...
                      const tableIdAsPropertyName =
                        this.sharedFunction.setIdToPropertyName(table.id);
          ...
          if (table?.actions) {
            ...
            const formIdAsPropertyName =
              this.sharedFunction.setIdToPropertyName(form.id);
            codeConstructor += `private ${formIdAsPropertyName}Builder: FormBuilder,private ${formIdAsPropertyName}Route: ActivatedRoute,`;
          }
        }
      }
      ...
      for (const key in object) {
        if (Object.prototype.hasOwnProperty.call(object, key)) {
          ...
          if (key === 'array') {
            const array = object[key];
            if (array) {
              const arrayClassName = this.sharedFunction.setIdToClassName(
                array.id,
              );
              const arrayPropertyName = this.sharedFunction.setIdToPropertyName(
                array.id,
              );
              ...
              codeElement += this.setFormBuilderElements(
                array.elements,
                formMetaObject,
              );
              codeElement += `})};`;
          ...
          if (object.select?.optionsObject) {
            const selectObjectName = this.sharedFunction.setIdToPropertyName(
              object.select?.name,
            );
  ...
  setTable = (
    ...
    if (table.elements)
      codeElements += this.setTableElements(table.elements, tableMetaObject);
    if (table.actions) {
      const form = table.actions;
      ...
      const formClassName = this.sharedFunction.setIdToClassName(form?.id);
      const formPropertyName = this.sharedFunction.setIdToPropertyName(
        form?.id,
      );
      const formMetaObject = {
  ...
  setTableElements = (
    ...
    elements.forEach(object => {
      if (object.row) {
        ...
        menus?.forEach(menu => {
          if (menu.dialog) {
            codeElement += this.setDialog(
              menu.dialog,
              tableMetaObject.propertyName,
            );
          }
        });
      }
        ...
        if (property === 'table') {
          ...
          for (const element in tableObject) {
            if (Object.prototype.hasOwnProperty.call(tableObject, element)) {
              const tableIdAsPropertyName =
                this.sharedFunction.setIdToPropertyName(tableObject.id);
              if (element === 'object') {
                const object = tableObject[element];

                codeTableObject += `${tableIdAsPropertyName}DataSource = ${JSON.stringify(
                  object,
                )};`;
              }
  ...
  setDialog = (dialog: DialogInterface, componentCallingDialogId: string) => {
    ...
    const dialogTemplateAsPropertyName =
      this.sharedFunction.setIdToPropertyName(dialog.templateFolder);
    const tableIdAsPropertyName = this.sharedFunction.setIdToPropertyName(
      componentCallingDialogId,
    );
    ...
    if (dialog.dialogDataInterface) {
      codeDialog += `,data:`;
      codeDialog += this.sharedFunction.objectToString(
