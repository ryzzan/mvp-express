export class SharedFunctions {
   ...
   */
  objectTransform = (
    objectOriginal: Array<Object>,
    attributesToUseFromObjectOriginal: Array<string>,
    alternativeAttributesOnNewObject: Array<string>,
    isArray?: boolean,
  ) => {
    let newObject = isArray ? '' : '[';
    objectOriginal.forEach((objectBeingChecked: any) => {
      newObject += `{`;
      for (let i = 0; i < attributesToUseFromObjectOriginal.length; i++) {
        ...
        if (Array.isArray(objectBeingChecked[attributeBeingChecked])) {
          ...
          newObject += this.objectTransform(
            objectBeingChecked[attributeBeingChecked],
            attributesToUseFromObjectOriginal,
            alternativeAttributesOnNewObject,
            true,
          );
          newObject += `]`;
        }
        if (
          objectBeingChecked[attributeBeingChecked] &&
          !Array.isArray(objectBeingChecked[attributeBeingChecked])
        ) {
