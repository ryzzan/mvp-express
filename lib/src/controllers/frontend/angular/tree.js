    ...
    function TreeAngular() {
        ...
        this.setTreeHtml = function (treeArray, nested) {
            ...
            treeArray.forEach(function (tree) {
                var codeTree = '', array = tree.elements, treeIdAsPropertyName = _this.shared.stringToLowerCamelCaseName(tree.id), treeIdAsClassName = _this.shared.stringToUpperCamelCase(tree.id);
        ...
        this.setTreeDirective = function (treeArray) {
            var codeTypescript = '', codeTree = '', codeConstructor = '', codeTreeObject = '', codeAction = '', array, treeIdAsPropertyName, treeIdAsClassName;
            treeArray.forEach(function (tree) {
                codeTree = '',
                    codeConstructor = 'constructor(';
                codeTreeObject = '',
                    array = tree.elements,
                    treeIdAsPropertyName = _this.shared.stringToLowerCamelCaseName(tree.id),
                    treeIdAsClassName = _this.shared.stringToUpperCamelCase(tree.id);
                ...
                array.forEach(function (element) {
                    if (element.nodes.type === frontend_1.RequestTypeEnum.Object) {
                        var objectArray = element.nodes.object;
                        codeTreeObject += _this.shared.objectTransform(objectArray, ['name', 'children'], ['name', 'children']);
